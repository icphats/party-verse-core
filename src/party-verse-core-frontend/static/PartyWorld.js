...
const InternalConfig = function (initConfig) {
  ...
  Config.prototype.update = function (opts) {
    ...
    this.executableBuffer = parse('executableBuffer', this.executableBuffer)
...
const Engine = (function () {
  ...
  Engine.loadBuffer = function (buffer, path, size) {
    if (loadPromise == null) {
      loadPath = path
      // loadPromise = preloader.loadPromise(buffer, size, true)
      var b = new Blob([buffer], { type: 'application/wasm' })
  ...
  function SafeEngine(initConfig) {
    const proto = /** @lends Engine.prototype */ {
       ...
       */
      init: function (basePath, exeBuffer) {
        if (initPromise) {
          return initPromise
        }
        if (loadPromise == null) {
          ...
          if (exeBuffer) {
            Engine.loadBuffer(exeBuffer, basePath, this.config.fileSizes[`${basePath}.wasm`])
          } else {
      ...
      startGame: function (override) {
        ...
        const exeBuffer = this.config.executableBuffer
        this.config.args = ['--main-pack', 'Tensura.pck'].concat(this.config.args)
        // Start and init with execName as loadPath if not inited.
        const me = this
        return Promise.all([this.init(exe, exeBuffer), this.preloadFile(pack, 'Tensura.pck')]).then(function () {
    ...
    Engine.prototype['loadBuffer'] = Engine.loadBuffer
  ...
  SafeEngine['loadBuffer'] = Engine.loadBuffer
